  ...
  uint8_t getControlData(const ByteMap (&map)[size]) const {
    ...
    for (size_t i = 0; i < size; i++) {
      /* Repeated line from the single-ByteMap function above. Apparently the
              constexpr stuff doesn't like being passed through nested
         functions. */
      dataOut |=
          (data.controlData[map[i].index] & map[i].mask) >> map[i].offset;
      // dataOut |= getControlData(map[i]);
    }
    return dataOut;
  }

  bool getControlBit(const BitMap map) const {
    return !(data.controlData[map.index] &
             (1 << map.position)); // Inverted logic, '0' is pressed
  }
...
private:
  ...
  bool controllerIDMatches() const;
...
public:
  BuildControllerClass() : ControllerMap(portData), portData() {}
...
// communication (ExtensionController) with a data instance (ExtensionData), but
// omits any controller-specific data maps.
using ExtensionPort =
    NintendoExtensionCtrl::BuildControllerClass<ExtensionController>;
